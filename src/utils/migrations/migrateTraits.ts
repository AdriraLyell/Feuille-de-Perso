...
export const migrateTraits = (parsed: MigratableData): void => {
    ...
    if (Array.isArray(page2.avantages) && page2.avantages.length > 0 && typeof page2.avantages[0] === 'string') {
        page2.avantages = page2.avantages.map((s: string) => ({ name: s || '', value: '' }));
    }
    if (Array.isArray(page2.desavantages) && page2.desavantages.length > 0 && typeof page2.desavantages[0] === 'string') {
        page2.desavantages = page2.desavantages.map((s: string) => ({ name: s || '', value: '' }));
    }

    // Padding to 28 elements
    const ensurePadding = (list: any[] | undefined) => {
        const current = list || [];
    ...
    page2.avantages = ensurePadding(page2.avantages);
    page2.desavantages = ensurePadding(page2.desavantages);
};

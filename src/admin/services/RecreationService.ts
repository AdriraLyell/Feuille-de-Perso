...
export const RecreationService = {
     ...
     * 1. Efface la création actuelle (si fullReset).
     * 2. Ajoute l'XP de remboursement (si fullReset).
     * 3. Réactive le mode création.
     */
    performRecreation(data: CharacterSheetData, refundAmount: number, currentRules: RulesData, fullReset: boolean = true): CharacterSheetData {
        let resultData: CharacterSheetData;

        if (fullReset) {
            // Step 1: Reset chirurgical (garde identité, social, équipement, traits, image)
            resultData = recreateCharacterStats(data);
            ...
            Object.keys(definedSkills).forEach(cat => {
                ...
                const existingSkills = resultData.skills[cat] || [];
                ...
                resultData.skills[cat] = newSkillList;
            });
            ...
            if (refundAmount > 0) {
                ...
                if (!resultData.xpLogs) resultData.xpLogs = [];
                resultData.xpLogs.push(recreationLog);
            }
        } else {
            // Soft copy

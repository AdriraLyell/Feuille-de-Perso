...
const CreationHUD: React.FC<CreationHUDProps> = ({ data, onValidate }) => {
  ...
  const renderBudgetGauge = (label: string, current: number, max: number) => {
      ...
      return (
          <div className={`flex flex-col bg-stone-800 rounded-lg border ${cardBorder} p-3 min-w-[140px] shadow-lg relative overflow-hidden group transition-all hover:-translate-y-1`}>
  ...
  return (
    <>
        <div className={`fixed bottom-0 left-0 right-0 z-[90] bg-stone-900 text-stone-100 shadow-[0_-5px_20px_rgba(0,0,0,0.5)] border-t-4 transition-colors duration-300 animate-in slide-in-from-bottom no-print ${hasErrors ? 'border-red-700' : 'border-amber-600'}`}>
            <div className="max-w-[1600px] mx-auto px-6 py-4 flex flex-col xl:flex-row items-center justify-between gap-6">
                ...
                <div className="min-w-[180px] flex justify-end">
                    <button 
                        ...
                        className={`group relative flex items-center gap-3 px-8 py-4 rounded font-bold shadow-xl transform transition-all hover:scale-105 active:scale-95 overflow-hidden ${
                            hasErrors 
